...
<?
	...
	$q = "select * from memoboard6 where num=$no";
	$r = mysql_fetch_array(mysql_query($q));
	
	if($r[pass] == $pass) {

	    $sql = "delete from memoboard6 where num=$no";
		$result = mysql_query($sql);
 
	    $sql2 = "delete from memoboard6 where linkno=$no";
		$result2 = mysql_query($sql2);

		if ($result) {
			echo "'memo' 삭제됨";

		   echo "<script> opener.parent.location='../popup6.php' </script>";
